...
# Specifies the location of the transmitted encrypted & hash appended images.
...
for transmission in "$transmission_dir"/*; do
    if [ -f "$transmission" ]; then
        ...
        if retransmit=false then
                echo "Transmission Successful, MD5 hashes match."
            else
                echo "Transmission Failed, MD5 hashes do NOT match."
            fi
    fi
done
